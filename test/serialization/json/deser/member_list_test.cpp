...
UNIT(ad_primary_list_test1, "test fas::json::deser::ad_primary_list advice")
{
  ...
  typedef type_list_n<
      ...
      member< n_foo5, attr<foo, foo::foo5type, &foo::foo5, array<integer> > >, // Игнорим в этом тесте
  ...
  >::type member_list_type;
  
  typedef primary_list< member_list_type > primary_list_type;
  
  std::string json = "\"foo1\" : 10 , \"foo2\" : \"test-foo2\" , \"foo3\" : \"test-foo3\" , \"bar1\" : 47, \"bar2\" : \"test-foo2\" }";
  
  bar b(0);
  ad_primary_list apr;
  apr(t, primary_list_type(), b, range(json) );
...
UNIT(ad_brute_list_test1, "test fas::json::deser::ad_brute_list advice")
{
  ...
  typedef brute_list< member_list_type > brute_list_type;
  
  std::string json = "\"foo1\" : 10 , \"foo2\" : \"test-foo2\" , \"foo3\" : \"test-foo3\" , \"bar1\" : 47, \"bar2\" : \"test-foo2\" }";
  
  bar b(0);
  ad_brute_list apr;
  apr(t, brute_list_type(), b, range(json) );
...
UNIT(ad_pair_test1, "test fas::json::deser::ad_brute_list advice")
{
  ...
  typedef type_list_n<
      ...
      member< n_foo2, attr<foo, std::string, &foo::foo2, string> >,
      member< n_foo3, attr<foo, foo::foo3type, &foo::foo3, string> >
  >::type member_list1_type;

  typedef type_list_n<
      member< n_foo5, attr<foo, foo::foo5type, &foo::foo5, array<integer> > >, // Игнорим в этом тесте
      member< n_bar1, attr<bar, bar::baz, &bar::bar1, attr<bar::baz, int, &bar::baz::baz1, integer> > >,
      member< n_bar2, attr<bar, foo, &bar::bar2, attr<foo, std::string, &foo::foo2, string> > >
  >::type member_list2_type;

  typedef ::fas::json::brute_pair< member_list1_type, member_list2_type > pair_type;
  
  std::string json = "\"foo1\" : 12 , \"foo2\" : \"test-foo2x\" , \"foo3\" : \"test-foo3x\" , \"bar1\" : 47, \"bar2\" : \"test-foo2\" }";
  
  bar b(0);
  ad_brute_pair apr;
  apr(t, pair_type(), b, range(json) );
...
UNIT(ad_member_list_test1, "test fas::json::deser::ad_member_list advice")
{
  using namespace ::fas;
  using namespace ::fas::testing;
  using namespace ::fas::json;
  ...
  typedef primary_list< member< n_foo1, attr<foo, int, &foo::foo1, integer> > > foo_list2;
  adfl(t, foo_list2(), f, range("\"foo1\":-1") );
  ...
  typedef primary_list< 
    ...
    >::type
  > foo_list3;
  adfl(t, foo_list3(), f, range("\"foo1\":-1,\"foo2\":\"test-foo2\"}") );
  t << equal<expect>(f.foo1, -1 ) << FAS_TESTING_FILE_LINE;
  ...
  typedef primary_list< 
    ...
    >::type
  > foo_list4;
  adfl(t, foo_list4(), f, range("\"foo1\":-1,\"foo2\":\"test-foo2\",\"foo3\":\"test-foo3\"") );
  t << equal<expect>(f.foo1, -1 ) << FAS_TESTING_FILE_LINE;
  t << equal<expect>(f.foo2, "test-foo2" ) << FAS_TESTING_FILE_LINE;
...
UNIT(ad_member_list_test2, "test fas::json::deser::ad_member_list advice")
{
  using namespace ::fas;
  using namespace ::fas::testing;
  using namespace ::fas::json;
  ...
  typedef primary_list< 
    ...
    >::type
  > bar_list1;
  adfl(t, bar_list1(), b, range("\"foo1\":-1,\"foo2\":\"test-foo2\",\"foo3\":\"test-foo3\",\"bar1\":42,\"bar2\":\"test-foo2\"") );
...
UNIT(ad_member_list_test3, "test fas::json::deser::ad_member_list advice")
{
  using namespace ::fas;
  using namespace ::fas::testing;
  using namespace ::fas::json;
  ...
  typedef primary_list< 
    ...
    >::type
  > bar_list1;
  adfl(t, bar_list1(), b, range("\"foo1\":-1,\"foo2\":\"test-foo2\",\"foo3\":\"test-foo3\","
...
BEGIN_SUITE(member_list_deserialize_suite, "member_list deserializer suite")
