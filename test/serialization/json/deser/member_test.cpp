...
UNIT(ad_member_test1, "test fas::json::deser::ad_member advice")
{
  using namespace ::fas;
  ...
  adm( t, foo_foo3(), f, range("\"foo3\":\"test-foo3\"") );
  ...
  adm( t, bar_baz_baz1(), b.bar1, range("\"bar1\":42") );
  ...
  adm( t, bar_bar1_baz1(), b, range("\"bar1\":42") );
  t << equal<expect>(b.bar1.baz1, 42 ) << FAS_TESTING_FILE_LINE;
  b=bar(0);

  typedef member< n_bar2, attr<bar, foo, &bar::bar2, attr<foo, std::string, &foo::foo2, string> > > bar_bar2_foo2;
  adm( t, bar_bar2_foo2(), b, range("\"bar2\":\"test-foo2\"") );
  t << equal<expect>(b.bar2.foo2, std::string("test-foo2") ) << FAS_TESTING_FILE_LINE;
  b=bar(0);

}

UNIT(ad_member_test2, "test fas::json::deser::ad_member advice")
{
  using namespace ::fas;
  using namespace ::fas::testing;
  using namespace ::fas::json;
...
UNIT(ad_member_test4, "test fas::json::deser::ad_member advice")
{
  using namespace ::fas;
  using namespace ::fas::testing;
  using namespace ::fas::json;
...
UNIT(ad_member_test5, "test fas::json::deser::ad_member advice")
{
  using namespace ::fas;
  using namespace ::fas::testing;
  using namespace ::fas::json;
...
BEGIN_SUITE(member_deserialize_suite, "member deserializer suite")
