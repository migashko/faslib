...
template<typename L, template<typename> class F, template<typename> class IF>
struct transform_if_impl1_t
  : transform_if_impl3_t<typename L::metatype, L, F, IF>
{};

#ifndef DISABLE_TYPE_LIST_SPEC

template<typename L, typename R, template<typename> class F, template<typename> class IF>
struct transform_if_impl1_t< type_list<L, R>, F, IF >
  : transform_if_impl2_t< type_list<L, R>, F, IF, IF<L>::type::value >
{
};

template<template<typename> class F, template<typename> class IF>
struct transform_if_impl1_t< empty_list, F, IF >
{
...
struct transform_if_impl2_t< type_list<L, R>, F, IF, B>
{
  typedef type_list< 
    typename F<L>::type,
    typename transform_if_impl1_t< R, F, IF>::type
  > type;
};

template<typename L, typename R, template<typename> class F, template<typename> class IF>
struct transform_if_impl2_t< type_list<L, R>, F, IF, false>
{
  typedef type_list< 
    L,
    typename transform_if_impl1_t< R, F, IF>::type
...
template<typename L, template<typename> class F, template<typename> class IF>
struct transform_if_impl3_t< metalist::type_list, L, F, IF>
  : transform_if_impl4_t< L, F, IF, IF< typename L::left_type >::type::value  >
{
};

template<typename L, template<typename> class F, template<typename> class IF>
struct transform_if_impl3_t< metalist::empty_list, L, F, IF>
{
  typedef empty_list type;
};
...
struct transform_if_impl4_t
{
  ...
  typedef type_list<
    ...
    typename transform_if_impl1_t< tail, F, IF>::type
  > type;
};

template<typename L, template<typename> class F, template<typename> class IF>
struct transform_if_impl4_t< L, F, IF, false>
{
  typedef typename L::left_type head;
  typedef typename L::right_type tail;
